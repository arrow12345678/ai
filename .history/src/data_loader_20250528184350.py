...
class ALPRDataLoader:
    ...
    def create_detection_dataset(self, split: str, shuffle: bool = True) -> tf.data.Dataset:
        """
        Create TensorFlow dataset for detection model.
        ...
        Returns:
            TensorFlow dataset
        """
        image_paths, annotations_list = self.load_dataset_split(split)
        ...
        def generator():
            ...
            for idx in indices:
                ...
                # Preprocess image
                image = self.preprocess_image(image, self.img_size)

                # Convert bboxes to tensor format
                if bboxes:
                    bbox_tensor = np.array(bboxes, dtype=np.float32)
                    labels_tensor = np.array(class_labels, dtype=np.int32)
                else:
                    bbox_tensor = np.zeros((0, 4), dtype=np.float32)
                    labels_tensor = np.zeros((0,), dtype=np.int32)

                yield image, {'bboxes': bbox_tensor, 'labels': labels_tensor}
    ...
    def create_ocr_dataset(self, split: str, shuffle: bool = True) -> tf.data.Dataset:
        """
        Create TensorFlow dataset for OCR model.
        ...
        Returns:
            TensorFlow dataset for OCR training
        """
        image_paths, annotations_list = self.load_dataset_split(split)
        ...
        def generator():
            ...
            for idx in indices:
                ...
                for ann in annotations:
                    if not ann['text']:  # Skip if no text annotation
                        continue
                    ...
                    # Extract and preprocess license plate
                    plate_img = image[y1:y2, x1:x2]
                    if plate_img.size == 0:
                        continue

                    plate_img = self.preprocess_ocr_image(plate_img)

                    # Encode text
                    encoded_text = self.encode_text(ann['text'])
                    text_length = len([c for c in ann['text'] if c in self.char_set])
        ...
        dataset = tf.data.Dataset.from_generator(
            generator,
            output_signature=(
                tf.TensorSpec(shape=(*self.ocr_img_size, 1), dtype=tf.float32),
                {
                    'text': tf.TensorSpec(shape=(10,), dtype=tf.int32),
                    'text_length': tf.TensorSpec(shape=(), dtype=tf.int32)
                }
            )
        )
    ...
    def split_dataset(self, source_dir: str, train_ratio: float = 0.8, val_ratio: float = 0.1):
        """
        ...
        Args:
            ...
            val_ratio: Ratio for validation set (test_ratio = 1 - train_ratio - val_ratio)
        """
        ...
        for split, files in [('train', train_files), ('val', val_files), ('test', test_files)]:
            split_img_dir = os.path.join(self.data_dir, split, 'images')
            split_label_dir = os.path.join(self.data_dir, split, 'labels')
            ...
            for img_file in files:
                ...
                for ext in ['.txt', '.xml']:
                    label_file = os.path.join(labels_dir, base_name + ext)
                    if os.path.exists(label_file):
                        dst_label = os.path.join(split_label_dir, base_name + ext)
